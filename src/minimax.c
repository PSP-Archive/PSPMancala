 ...
 */
static void   (*loc_pushBoard)(void);
static void   (*loc_popBoard)(void);
static PMove *(*loc_getMoves)(Player, int *);
static Player (*loc_doMove)(Player, PMove);
static void   (*loc_undoMove)(Player, PMove);
static Score  (*loc_evalBoard)(Player);
	      ...
	      )
{
     ...
     */
    loc_pushBoard = fncPushBoard;
    loc_popBoard  = fncPopBoard;
    loc_getMoves  = fncGetMoves;
    loc_doMove    = fncDoMove;
    loc_undoMove  = fncUndoMove;
    loc_evalBoard = fncEvalBoard;
     ...
     */
    bestMove = NULL;
    move = loc_getMoves(player, &numMoves);
    bestValue = -INF_SCORE;
    for (q = 0; q < numMoves; q++) {
...
print_board()
{
  ...
  for (player = 0; player < 2; player++) {
    for (hole = 0; hole < MAX_HOLES; hole++) {
      fprintf(stdout, "[%d] [%d] : %d\n", player, hole, getHole(player, hole));
    }
  }
}
# endif
