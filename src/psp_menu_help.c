...
# define HELP_LINE_BY_PAGE   26
# define HELP_CHAR_BY_LINE   79
...
psp_initialize_help(void)
{
  ...
  snprintf(FileName, MAX_PATH, "%s/help.txt", psp_homedir );
...
psp_display_screen_help(void)
{
  ...
  while ((index < HELP_LINE_BY_PAGE) && (help_line < psp_help_size))  {
    ...
    psp_sdl_back2_print(x, y, buffer, PSP_MENU_HELP_COLOR);
    y += y_step;
    index++;
    help_line++;
  }
